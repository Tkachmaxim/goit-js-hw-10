...
function onSearchInput(e) {
  ...
  if (nameOfCountry) {
    restCountries(nameOfCountry)
      .then(createMarkUp)
      .catch(error => Notify.failure('Ooops, there is no country with that name'));
  }
}
...
function createMarkUp(countries) {
  ...
  const markUpList = countries
    .map(({ flags: { svg }, name: { official } }) => {
      return `<li><img src="${svg}" alt="${official}" width="100" height="50"/>&nbsp &nbsp${official}</li>`;
    })
    .join('');
